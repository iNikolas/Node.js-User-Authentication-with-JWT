...
module.exports = {
    refreshAccessToken: async (req, res, next) => {
        try {
            ...
            const refreshTokenRequest = await pool.query(`SELECT * FROM refreshtokens WHERE TOKEN = $1`, [refreshToken])
            ...
            jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET, (err, user) => {
                ...
                const userSharedData = {name: user.name, uid: user.uid, rights: user.rights}
    ...
    getAllUsers: async (req, res, next) => {
        try {
            const allUsersRequest = await pool.query(`SELECT users.uid, name, password, rights, token FROM users LEFT JOIN refreshtokens ON users.uid = refreshtokens.user_uid`)
    ...
    getUser: async (req, res, next) => {
        try {
            ...
            const userRequest = await pool.query(`SELECT users.uid, name, password, rights, token FROM users LEFT JOIN refreshtokens ON users.uid = refreshtokens.user_uid WHERE users.uid = $1`, [id])
    ...
    updateUser: async (req, res, next) => {
        try {
            ...
            if (!newName && !newPassword) throw new ServerError('Lack of data to process. Please provide at least newName or newPassword, not empty fields!', 400, 'Bad Request')

            const userRequest = await pool.query(`SELECT * FROM users WHERE uid = $1`, [id])
            const userRespond = userRequest.rows[0]
            ...
            if (await bcrypt.compare(oldPassword, userRespond.password)) {
                ...
                const password = newPassword ? await bcrypt.hash(newPassword, 10) : userRespond.password

                await pool.query(`UPDATE users SET name = $1, password = $2 WHERE uid = $3`, [name, password, id])

                res.status(204)
                res.send()

            } else {
                throw new ServerError('You have typed in wrong old password!', 403, 'Forbidden')
            }
        } catch (error) {
            next(error)
        }
    ...
    createNewUser: async (req, res, next) => {
        try {
            ...
            const newUserRequest = await pool.query(`INSERT INTO ${type} (uid, name, password, rights) VALUES(uuid_generate_v4(), $1, $2, 'user') RETURNING *`, [name, hashedPassword])
    ...
    deleteUser: async (req, res, next) => {
        ...
        try {
            ...
            if (rights !== 'admin') throw new ServerError('You do not have access rights to the content!', 403, 'Forbidden')
    ...
    loginUser: async (req, res, next) => {
        try {
            ...
            const userRequest = await pool.query(`SELECT * FROM users WHERE name = $1`, [name])
            ...
            if (await bcrypt.compare(password, user.password)) {
                ...
                const saveRefreshTokenRequest = await pool.query(`INSERT INTO refreshTokens (uid, token, user_uid) VALUES (uuid_generate_v4(), $1, $2)
                                                                  ON CONFLICT (user_uid) DO UPDATE SET token = EXCLUDED.token RETURNING *
                `, [refreshToken, id])
                const saveRefreshToken = saveRefreshTokenRequest.rows[0]
                ...
                const resData = {
                    ...
                    meta: {
                        accessToken,
                        refreshToken: saveRefreshToken.token
                    }
                }
    ...
    logoutUser: async (req, res, next) => {
        try {
            ...
            const deleteRefreshTokenRequest = await pool.query(`DELETE FROM refreshtokens WHERE token = $1`, [refreshToken])
